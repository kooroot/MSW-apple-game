...
- **GameSession lifetime is map-scoped** → it is a `@Component` on the `AppleGame.map` entity (placed via `MapBuilder.upsertComponent`), NOT a `@Logic`. `OnMapEnter` / `OnMapLeave` fire only on Components, never on Logic. The four UI controllers (`BoardController`, `HudController`, `ResultController`, `LeaderboardController`) are **global `@Logic` singletons (ClientOnly)**, called as `_BoardController` / `_HudController` / `_ResultController` / `_LeaderboardController`; they are **NOT** `script(...)` entities and are **NOT** added via `b.script(...)`. Each reaches its `.ui` entities only through **injected UUID property bindings** (`b.write(path, { bind })` / `injectBindings`, builder-protocol §3.6) — a `@Logic` has no `self.Entity` and is not part of the `.ui` tree. (msw-scripting §3.2, spec §5, correction #6)
