...
- `@ExecSpace("Server") method void SubmitRun(string token, table moves, integer claimedScore, number clientElapsed)` — validates: (1) token exists, belongs to `senderUserId`, not yet consumed; (2) `clientElapsed` and server elapsed since issuance both within the 120s game window + slack (reject absurd timing); (2b) payload shape (move-count cap, coord ranges incl. numeric/integer type guards, monotonic timestamps); (3) `local r = _PuzzleCore:Replay(session.seed, moves)`; accept iff `r.ok and r.score == claimedScore`. On accept: mark token consumed; for `mode=="single"` update `personalBest` (keep-max), while `mode=="ranked"` only **reads** `personalBest` read-only (never writes it — the single best is shown read-only on the ranked result popup); and for `mode=="ranked"` call `_LeaderboardService:SubmitScore(senderUserId, r.score)` (Plan 3, nil-guarded seam). Reply via Client RPC.
- `@ExecSpace("Client") method void ReceiveSubmitResult(boolean accepted, integer finalScore, integer personalBest, string reason)` — server→caller. `reason` ∈ `"ok"|"bad_token"|"timing"|"replay_mismatch"|"already"|"bad_payload"`. (Plan 4's Result-UI reason map must enumerate all six; `bad_payload` fires on out-of-range/non-numeric coords or move-count overflow.)
...
- `method boolean TryConsumeDaily(string userId, integer dateKey)` — ServerOnly; on the user's UserDataStorage, GetAndWait `rankAttempt:{dateKey}` (verified GATE B: a missing key returns `ERR_OK` + **empty string**, NOT `ERR_NOT_FOUND` — branch on the VALUE, not the errCode); if empty/missing `SetAndWait("1")` (UpdateAndWait CANNOT create a missing key), reserving UpdateAndWait CAS for an existing unexpected value. Returns true if newly consumed, false if already `"1"` or on error (fail-closed). An in-memory per-(user,day) in-flight guard (`_T`) serializes concurrent same-instance consumes (closes the TOCTOU window under the MSW one-instance-per-user model; cross-instance atomicity would need a billed Transact and is an accepted residual).
- `method integer GetPersonalBest(string userId)` / `method integer SetPersonalBestIfHigher(string userId, integer score)` — UserDataStorage `personalBest` (string-serialized integer). `SetPersonalBestIfHigher` is keep-max with a dirty check and returns the resulting best (single integer — MSW method bodies cannot declare multi-return; `[LEA-3015]`).
